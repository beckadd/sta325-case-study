...
With this evidence settled on the following predictive models for the raw moments:


2. $\log{E[X^2]} = (5.599, -28.268, -10.562) \cdot (St, Re, Fr) + (-4.953, (St^2, Re^2, Fr^2) + (St, Re, 0) + (St, 0, Fr) + (0, Re, Fr)$
3. $\log{E[X^3]}$
4. $\log{E[X^4]}$
...
```{r prediction-with-selected-models}
tested <- test %>%
  mutate(
    Fr = case_when(
      Fr == Inf ~ 1,
...
tested$R_moment_1 = predict(data$nl.fit.two_degree_lt_selected[[1]], tested)
tested$R_moment_2 = predict(data$nl.fit.two_degree_lt_selected[[2]], tested)
tested$R_moment_3 = predict(data$nl.fit.two_degree_lt_selected[[3]], tested)
tested$R_moment_4 = predict(data$nl.fit.two_degree_lt_selected[[4]], tested)
